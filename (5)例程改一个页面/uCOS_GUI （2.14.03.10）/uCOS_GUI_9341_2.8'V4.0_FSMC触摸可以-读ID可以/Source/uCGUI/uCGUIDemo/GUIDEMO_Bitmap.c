...
static const GUI_BITMAP _bmLadyBug = {
  160,               /* XSize */
  161,               /* YSize */
  40,                /* BytesPerLine 每行的字节 */
  GUI_COMPRESS_RLE4, /* BitsPerPixel每一个像素占比特 */
  _acLadyBug,        /* Pointer to picture data (indices) 指向图片数据*/
  &_PalLadyBug       /* Pointer to palette 指针到调色板 */
  ,GUI_DRAW_RLE4
};
...
void GUIDEMO_Bitmap(void) 
{
	...
	GUI_Clear();
		...
		WM_EnableMemdev(WM_HBKWIN);		//启用存储设备的重绘功能
		/*参数  含义 
		hWin  窗口的句柄。 
		*/
	#endif
