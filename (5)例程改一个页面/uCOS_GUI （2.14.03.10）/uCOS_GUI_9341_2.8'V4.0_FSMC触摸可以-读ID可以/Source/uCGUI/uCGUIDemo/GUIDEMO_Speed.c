...
/************************************************************************/
	...
	for (i = 0; (((t + 8000) - (int)GUI_GetTime()) > 0) && !GUIDEMO_CheckCancel(); i++)
	{
		...
		/* Allow short breaks so we do not use all available CPU time ...五颜六色方块显示 */
	}
	t = (GUI_GetTime() - t) / 100;
	GUI_Clear();
 /************************************************************************/
...
/************************************************************************/
	...
	#if GUIDEMO_LARGE
		GUI_SetFont(&GUI_FontComic24B_ASCII);			   //设置用于文字输出的字体
	#else
	...
	if (t == 0)
	t++;
	GUI_DispDecMin(10 * (NumPixels / t));	//在当前坐标显示最少数量字符的十进制数值
	GUIDEMO_Wait();
}
