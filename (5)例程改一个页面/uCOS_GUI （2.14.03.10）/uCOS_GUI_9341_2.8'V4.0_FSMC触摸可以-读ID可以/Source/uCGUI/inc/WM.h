...
/*********************************************************************
*
*           Window create flags. (窗口创建旗帜)
* These flags can be passed to the create window(这些标志可以被传递到创建窗口)
* function as flag-parameter. The flags are combinable using the(函数作为国旗参数。标志是可以化合的使用)
* binary or operator.	  (二进制运算)
*/
...
void WM_Init      (void);
int  WM_Exec      (void);    /* Execute all jobs ... Return 0 if nothing was done.(通过执行回调函数（所有工作）重绘无效窗口。) */
...
void    WM_InvalidateWindow          (WM_HWIN hWin);    /*使一个窗口无效*/
...
GUI_COLOR WM_SetDesktopColorEx(GUI_COLOR Color, unsigned int LayerIndex);	   //LayerIndex:图层索引
...
*/
  ...
  #define WM_ExecIdle       WM_Exec					//通过执行回调函数（所有工作）重绘无效窗口
  ...
  #define WM_SetForegroundWindow    WM_BringToTop			//在其同胞窗口之前放置一个窗口。
