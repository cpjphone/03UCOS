...
void GPIO_Config(void)
{
  ...
  GPIO_Init(GPIOD, &GPIO_InitStructure);  //初始化PB口
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD;  //下拉
	...
	//GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;  //配置成推挽式输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz; //输出模式下 I/O输出速度 50M HZ
  GPIO_Init(GPIOE, &GPIO_InitStructure);  //初始化PB口
...
void PWM2_Configuration(void)
{
  TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	...
	TIM_TimeBaseStructure.TIM_Prescaler =47; //设置用来作为TIMx时钟频率除数的预分频值  不分频(0x0000--0xffff)即(0--65536)
